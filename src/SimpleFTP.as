package  {
  ...
  public class SimpleFTP {
    public static function putFile(host:String, user:String, pass:String, 
                                   path:String, contents:ByteArray,
                                   listener:Function) {
    ...
    private var path:String, contents:ByteArray;
    ...
    private function putFile(path:String, contents:ByteArray,
    ...
    private function response(res:String, contents:ByteArray = null):void {
      step = 11;
      if (put)
        listener(res);
      else
        ...
        if (st == "227") {
          ...
          dataSocket.addEventListener(ProgressEvent.SOCKET_DATA,
            function (event:ProgressEvent):void {
             // contents=dataSocket.readBytes(dataSocket.bytesAvailable);
            });
          dataSocket.connect(dataIP, dataPort);
          write("RETR " + path);
        } else
